"""
SIMULADOR HIDRÁULICO PROFESIONAL v11.1
...
class ProfessionalSimulator:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Simulador Hidráulico Profesional v11.1 - Emanuel & Arita")
...
"""
    ...
    def solve_for_y(self, Q, b, n, S):
        """Resuelve para y"""
        y_min = 1e-6
        y_max = max(10.0, 10.0 * Q / b if b > 1e-6 else 100.0)
        try:
            y = brentq(self.manning_residual, y_min, y_max,
                      args=(Q, b, n, S), xtol=1e-9)
            return y
        except:
            raise ValueError("No se pudo resolver para y")
...
Régimen: {p['regimen']} (Fr = {p['Fr']:.3f})
    ...
    def calculate_gate(self):
        ...
        try:
            ...
            y1 = Q / (b * math.sqrt(2 * self.g * a))
            y2 = Cc * a
            Cd = Cc / math.sqrt(1 + (y2 / y1))
            V2 = Q / (b * y2)
