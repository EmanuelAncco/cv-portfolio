                        ...
                        if current_win_size < 3: # Si la ventana es muy corta (1 o 2 puntos)
                           ssim_val = 1.0 if np.allclose(sig_in, sig_out) else 0.0 # Comparación directa
                            ...
                            try:
                                ...
                                local_logger.warning(f"SSIM calculation failed for window {processed_windows + i}, sensor {n+1}. Setting to 0. Error: {ve}")
...
def plot_reconstruction_sample_translated(original_scaled, reconstructed_scaled, scaler, window_idx, output_dir, prefix):
    """
...
def plot_sensor_reconstruction_samples(all_originals, all_reconstructions, scaler, num_sensors, output_dir, prefix):
    ...
    for i in range(num_sensors):
        ...
        try:
            ...
            local_logger_plot.error(f"Error selecting random indices for sensor {i+1} ({prefix}): {e}. Skipping sensor.")
            continue
        ...
        try:
            ...
            local_logger_plot.error(f"Error during inverse_transform in plot_sensor_reconstruction_samples: {e}. Plotting scaled data.")
        ...
        ax_sig1.plot(reconstructed_sample1_inv, label=f'Reconstructed (Sample {idx1})', color=color1, linestyle='--', linewidth=1.5, alpha=1.0)
        ...
        ax_sig2.plot(reconstructed_sample2_inv, label=f'Reconstructed (Sample {idx2})', color=color2, linestyle='--', linewidth=1.5, alpha=1.0)
...
def plot_mse_comparison(mse_healthy_per_sensor_avg, mse_damage_per_sensor_avg, output_dir):
    ...
    for i, v in enumerate(plot_data_damage):
        if use_log_scale:
            ...
            # CORRECCIÓN: Usar get_ylim() para asegurar que la posición del texto esté dentro de los límites
...
def plot_damage_localization(losses_damage_per_sensor, losses_healthy_per_sensor, output_dir):
    ...
    # --- Panel Superior: Violin Plot + Box Plot Error con Daño (Escala Log) --- MODIFICADO ---
    ...
    if not data_list or pd.DataFrame(data_list)['MSE'].max() <= 1e-11 : # Chequear si hay datos válidos
        ...
        sns.violinplot(x='Sensor', y='MSE', data=df_damage, ax=axes[0],
                       ...
                       inner=None, # Quitar elementos internos del violín
                       cut=0, scale='width', linewidth=1.5, alpha=0.7) # Hacer violín semi-transparente

        # --- ***** INICIO DE LA MODIFICACIÓN (AÑADIR BOXPLOT) ***** ---
        # Superponer el box plot
        sns.boxplot(x='Sensor', y='MSE', data=df_damage, ax=axes[0],
                    showcaps=False, # No mostrar 'caps' en los bigotes
                    boxprops={'facecolor':'None', "zorder": 10}, # Caja sin relleno, encima
                    showfliers=False, # No mostrar outliers (ya están implícitos en el violín/strip)
                    whiskerprops={'linewidth':2, "zorder": 10, 'color':'black'}, # Bigotes negros y gruesos
                    medianprops={'linewidth':2, "zorder": 10, 'color':'black'}, # Mediana negra y gruesa
                    width=0.3) # Ancho de la caja
        # --- ***** FIN DE LA MODIFICACIÓN ***** ---

        # Superponer el strip plot (opcional, puede hacer el gráfico muy denso)
        # sns.stripplot(x='Sensor', y='MSE', data=df_damage, ax=axes[0],
        #               color='black', alpha=0.05, size=1.5, jitter=0.3) # Puntos más pequeños y transparentes si se usan

        # --- TEXTO TRADUCIDO ---
        axes[0].set_title('Reconstruction Error Distribution (Damage Data - Violin + Box)') # Título actualizado
        axes[0].set_ylabel('Mean Squared Error (MSE) - Log Scale')
        axes[0].set_yscale('log')
        axes[0].grid(True, linestyle=':')

        # Ajustar límites Y basado en percentiles (ahora un poco más estrictos: 0.5 y 99.5)
        valid_mse = df_damage['MSE'][np.isfinite(df_damage['MSE'])]
        if len(valid_mse) > 0:
            p_low = np.percentile(valid_mse, 0.5) # Percentil 0.5
            p_high = np.percentile(valid_mse, 99.5) # Percentil 99.5
            ...
            local_logger_plot.info(f"Damage localization plot (top): Setting Y-limits based on percentiles [0.5, 99.5]: [{min_mse_plot:.2e}, {max_mse_plot:.2e}]")
    ...
    if len(valid_amp) > 0:
        ...
        for i, v in enumerate(amplification_factor):
            if np.isfinite(v): # Solo añadir texto si el valor es finito
                axes[1].text(i, v + 0.02 * max(max_amp, 1e-9), f"{v:.2f}", ha='center', va='bottom', fontsize=9.5) # Ajustar posición y tamaño texto
    else:
        axes[1].set_ylim(bottom=0) # Default si no hay datos válidos


    plt.tight_layout(rect=[0, 0.03, 1, 0.96])
    # Actualizar nombre de archivo para reflejar el cambio
    filename = os.path.join(output_dir, "damage_localization_analysis_violinbox.png")
    try:
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        local_logger_plot.info(f"Damage localization graph (Violin+Box) saved in: {filename}")
    except Exception as e:
        local_logger_plot.error(f"Error saving graph {filename}: {e}")
    plt.close(fig)
...
def plot_error_distribution_kde(losses_healthy, losses_damage, output_dir):
    ...
    if len(log_losses_healthy) > 1:  # KDE necesita al menos 2 puntos
        # --- TEXTO TRADUCIDO ---
        sns.kdeplot(log_losses_healthy, label='Healthy', color=palette_scenario[0], fill=True, bw_adjust=0.5, alpha=0.6) # Usar paleta
    else:
    ...
    if len(log_losses_damage) > 1:  # KDE necesita al menos 2 puntos
        # --- TEXTO TRADUCIDO ---
        sns.kdeplot(log_losses_damage, label='Damage', color=palette_scenario[1], fill=True, bw_adjust=0.5, alpha=0.6) # Usar paleta
    else:
...
def plot_error_timeseries(losses_healthy, losses_damage, output_dir):
    ...
    if losses_healthy is not None and losses_healthy.size > 0:
        ...
        if len(valid_healthy_indices) > 0:
            # --- TEXTO TRADUCIDO ---
            axes[0].plot(valid_healthy_indices, losses_healthy[valid_healthy_indices], label='Healthy Error', color=palette_scenario[0], # Usar paleta
    ...
    if losses_damage is not None and losses_damage.size > 0:
        ...
        if len(valid_damage_indices) > 0:
            # --- TEXTO TRADUCIDO ---
            axes[1].plot(valid_damage_indices, losses_damage[valid_damage_indices], label='Damage Error', color=palette_scenario[1], # Usar paleta
                         alpha=0.7, linewidth=0.5)  # Línea más fina
            axes[1].set_yscale('log')
...
def plot_error_heatmap(losses_healthy_per_sensor, losses_damage_per_sensor, output_dir):
    ...
    # --- MODIFICACIÓN YLIM (VMIN/VMAX) ---
    # Usar percentiles para vmin y vmax para robustez contra outliers
    vmin = np.percentile(all_data_positive, 0.5)
    vmax = np.percentile(all_data_positive, 99.5) # Usar percentil 99.5 como límite superior

    # Asegurar vmin > 0 para LogNorm
    vmin = max(vmin, 1e-12)

    # Usar LogNorm para mejor visualización si hay outliers
    # Manejar caso donde vmin y vmax son iguales o muy cercanos
    if vmin >= vmax:
        ...
        if vmin_plot == 0 and vmax_plot == 0: # Caso extremo
            vmin_plot = -1
            vmax_plot = 1
        norm = plt.Normalize(vmin=vmin_plot, vmax=vmax_plot)
        cbar_label = 'MSE (Linear Scale)'
        local_logger_plot.warning(f"Heatmap vmin ~= vmax ({vmin:.2e}). Using linear scale near value.")
    else:
        norm = LogNorm(vmin=vmin, vmax=vmax) # Usar vmax_plot
...
# (Modificado para usar Violinplots + Boxplots)
def plot_error_statistics_per_sensor(losses_healthy_per_sensor, losses_damage_per_sensor, output_dir):
    """
    Genera gráficos de violín+boxplot comparativos de la distribución de errores por sensor
    ...
    local_logger_plot.info("Generating statistical error graph per sensor (Violinplots + Boxplots)...") # Mensaje actualizado
    ...
    fig, axes = plt.subplots(1, 2, figsize=(18, 7), sharey=False)  # EJES Y SEPARADOS

    # --- TEXTO TRADUCIDO ---
    fig.suptitle('Statistical Distribution of Reconstruction Error per Sensor', fontsize=16)

    # --- Panel Datos Sanos: Violinplot + Boxplot --- MODIFICADO ---
    if not df_healthy.empty and df_healthy['MSE'].max() > 1e-11: # Chequear datos válidos
        sns.violinplot(x='Sensor', y='MSE', data=df_healthy, ax=axes[0],
                       palette=sensor_colors_mapped, # Usar colores mapeados para violines
                       inner=None, # Quitar elementos internos del violín
                       cut=0, scale='width', linewidth=1.5, alpha=0.7) # Violín semi-transparente
        ...
        # Optional Stripplot (puede ocultar el boxplot si hay muchos puntos)
        # sns.stripplot(x='Sensor', y='MSE', data=df_healthy, ax=axes[0],
        #               color='black', alpha=0.05, size=1.5, jitter=0.3)

        # --- TEXTO TRADUCIDO ---
        axes[0].set_title('Healthy Data (Baseline - Violin + Box)') # Título actualizado
        axes[0].set_ylabel('Mean Squared Error (MSE) - Log Scale')
        axes[0].set_yscale('log')
        axes[0].grid(True, linestyle=':')

        # Ajustar límites Y basado en percentiles (0.5 y 99.5)
        ...
        if len(valid_mse_h) > 0:
            ...
            local_logger_plot.info(f"Error statistics plot (Healthy): Setting Y-limits based on percentiles [0.5, 99.5]: [{min_mse_plot_h:.2e}, {max_mse_plot_h:.2e}]")
            axes[0].set_ylim(bottom=min_mse_plot_h, top=max_mse_plot_h)
    else:
        axes[0].set_title('Healthy Data (No positive data or too small)')


    # --- Panel Datos con Daño: Violinplot + Boxplot --- MODIFICADO ---
    if not df_damage.empty and df_damage['MSE'].max() > 1e-11: # Chequear datos válidos
        sns.violinplot(x='Sensor', y='MSE', data=df_damage, ax=axes[1],
                       palette=sensor_colors_mapped, # Usar colores mapeados para violines
                       inner=None, cut=0, scale='width', linewidth=1.5, alpha=0.7)
        ...
        # --- TEXTO TRADUCIDO ---
        axes[1].set_title('Damage Data (Violin + Box)') # Título actualizado
        ...
        if len(valid_mse_d) > 0:
            ...
            local_logger_plot.info(f"Error statistics plot (Damage): Setting Y-limits based on percentiles [0.5, 99.5]: [{min_mse_plot_d:.2e}, {max_mse_plot_d:.2e}]")
    ...
    try:
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        local_logger_plot.info(f"Statistical error violin+boxplot saved in: {filename}")
    except Exception as e:
        local_logger_plot.error(f"Error saving statistical violin+boxplot: {e}")
    plt.close(fig)


# --- ************ NUEVAS FUNCIONES DE PLOTEO (PAPER FIGS. 9 & 10) ************ ---

def plot_evaluation_scatters_and_boxes(healthy_losses_global, healthy_ssim_global, damage_losses_global, damage_ssim_global, output_dir):
    """
    Replica la estructura de la Figura 9 del paper:
    (a) Scatter Error vs. Samples
    (b) Scatter SSIM vs. Samples
    (c) Violinplot+Boxplot Error vs. Scenario  <--- MODIFICADO
    (d) Violinplot+Boxplot SSIM vs. Scenario  <--- MODIFICADO
    """
    ...
    # --- (a) Scatter Error vs. Samples ---
    axes[0, 0].scatter(indices_healthy, healthy_losses_global, label='Healthy', alpha=0.5, s=10, color=palette_scenario[0]) # Usar paleta
    axes[0, 0].scatter(indices_damage, damage_losses_global, label='Damage', alpha=0.5, s=10, color=palette_scenario[1]) # Usar paleta
    ...
    # Ajuste YLIM con percentiles (0.5, 99.5)
    ...
    if len(valid_losses) > 0:
        ...
        if max_error_all / min_error_all > 1000:
            ...
            local_logger_plot.info(f"Fig. 9 plot (a): Setting Y-limits based on percentiles [0.5, 99.5]: [{min_mse_plot:.2e}, {max_mse_plot:.2e}]")
            axes[0, 0].set_ylim(bottom=min_mse_plot, top=max_mse_plot)


    # --- (b) Scatter SSIM vs. Samples ---
    axes[0, 1].scatter(indices_healthy, healthy_ssim_global, label='Healthy', alpha=0.5, s=10, color=palette_scenario[0]) # Usar paleta
    axes[0, 1].scatter(indices_damage, damage_ssim_global, label='Damage', alpha=0.5, s=10, color=palette_scenario[1]) # Usar paleta
    ...
    if full_df_pos_mse.empty or full_df_pos_mse['MSE'].max() <= 1e-11:
        ...
        # --- ***** AÑADIR BOXPLOT ***** ---
        sns.boxplot(x='Scenario', y='MSE', data=full_df_pos_mse, ax=axes[1, 0],
                    showcaps=False, boxprops={'facecolor':'None', "zorder": 10},
                    showfliers=False, whiskerprops={'linewidth':2, "zorder": 10, 'color':'black'},
                    medianprops={'linewidth':2, "zorder": 10, 'color':'black'}, width=0.3)
        # --- ***** FIN BOXPLOT ***** ---

        axes[1, 0].set_title('(c) Error Distribution by Scenario (Violin + Box)') # Título actualizado
        ...
        if len(valid_mse_c) > 0:
            ...
            local_logger_plot.info(f"Fig. 9 plot (c): Setting Y-limits based on percentiles [0.5, 99.5]: [{min_mse_plot_c:.2e}, {max_mse_plot_c:.2e}]")
    ...
    # --- ***** AÑADIR BOXPLOT ***** ---
    sns.boxplot(x='Scenario', y='SSIM', data=full_df, ax=axes[1, 1],
                showcaps=False, boxprops={'facecolor':'None', "zorder": 10},
                showfliers=False, whiskerprops={'linewidth':2, "zorder": 10, 'color':'black'},
                medianprops={'linewidth':2, "zorder": 10, 'color':'black'}, width=0.3)
    # --- ***** FIN BOXPLOT ***** ---

    axes[1, 1].set_title('(d) SSIM Distribution by Scenario (Violin + Box)') # Título actualizado
    ...
    try:
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        local_logger_plot.info(f"Fig. 9 style plot (Violin+Box) saved in: {filename}")
    except Exception as e:
        local_logger_plot.error(f"Error saving Fig. 9 style plot {filename}: {e}")
    plt.close(fig)


def plot_ssim_vs_error_cluster(healthy_losses_per_sensor, healthy_ssim_per_sensor, damage_losses_per_sensor, damage_ssim_per_sensor, num_nodes, output_dir):
    """
    ...
    for i in range(num_nodes):
        ...
        axes[0].scatter(h_loss, h_ssim, label='Scenario: Healthy', color=palette_scenario[0], alpha=0.3, s=15) # Usar paleta
        axes[0].scatter(d_loss, d_ssim, label='Scenario: Damage', color=palette_scenario[1], alpha=0.3, s=15) # Usar paleta
        ...
        axes[0].legend()
        axes[0].grid(True, linestyle=':')
        # Considerar escala log para el eje X (Error) y ajustar límites con percentiles (0.5, 99.5)
        ...
        if len(all_losses_sensor)>0:
            min_loss_s = max(np.min(all_losses_sensor[all_losses_sensor > 0]), 1e-12)
            ...
            if max_loss_s / min_loss_s > 100:
                ...
                p_low_x = np.percentile(all_losses_sensor[all_losses_sensor > 0], 0.5)
        ...
        axes[1].plot([h_loss_mean, d_loss_mean], [h_ssim_mean, d_ssim_mean], marker='o', markersize=10, linestyle='-', linewidth=2, color='darkgrey') # Linea gris oscuro
        axes[1].scatter(h_loss_mean, h_ssim_mean, label=f'Healthy Avg (E: {h_loss_mean:.2e}, S: {h_ssim_mean:.2f})', color=palette_scenario[0], s=120, zorder=5, edgecolor='black') # Puntos más grandes
        axes[1].scatter(d_loss_mean, d_ssim_mean, label=f'Damage Avg (E: {d_loss_mean:.2e}, S: {d_ssim_mean:.2f})', color=palette_scenario[1], s=120, zorder=5, edgecolor='black') # Puntos más grandes
        ...
        # Ajustar límites X para que coincidan (o sean similares) a (a)
...
def plot_training_history(log_file_path, output_dir):
    ...
    epoch_pattern = re.compile(r"Epoch (\d+)/\d+ -> Train Loss: ([\d.eE+-]+), Val Loss: ([\d.eE+-]+)") # Mejorado para notación científica
            ...
            for line in f:
                ...
                if match:
                    try:
                        ...
                        local_logger_plot.warning(f"Could not parse loss values in line: {line.strip()}. Skipping line.")
                        continue
    except FileNotFoundError:
        local_logger_plot.error(f"Training log file not found at: {log_file_path}. Skipping training history plot.")
        return
    except Exception as e:
        local_logger_plot.error(f"Error parsing training log file {log_file_path}: {e}. Skipping training history plot.")
        return
    ...
    plt.plot(epochs, train_losses, label='Train Loss', marker='o', linestyle='-', color='#e41a1c', markersize=4) # Rojo (Set1)
    plt.plot(epochs, val_losses, label='Validation Loss', marker='x', linestyle='--', color='#377eb8', markersize=5) # Azul (Set1)
...
def plot_bridge_graph(edge_index_numpy, output_dir, node_values=None, cmap=plt.cm.viridis, title_suffix="", filename_suffix="", vmin=None, vmax=None):
    """
    ...
    nx.draw(G, pos, ax=ax, with_labels=True, labels=labels, node_color=node_color_param,
            node_size=800, font_size=12, font_color='white', font_weight='bold', edge_color='gray', width=1.5) # Ajustes visuales
...
def plot_graph_heatmap_comparison(edge_index_numpy, healthy_values, damage_values, output_dir):
    ...
    # Filtrar inf/nan
    healthy_values = np.nan_to_num(healthy_values, nan=0.0, posinf=np.finfo(np.float32).max, neginf=np.finfo(np.float32).min)
    damage_values = np.nan_to_num(damage_values, nan=0.0, posinf=np.finfo(np.float32).max, neginf=np.finfo(np.float32).min)
...
def run_inference_and_plot(model_dir, base_healthy_dir, damage_data_dir=None):
    ...
    try:
        ...
        plot_bridge_graph(edge_index.cpu().numpy(), results_dir, title_suffix="(Structure Only)", filename_suffix="_structure")
        ...
        if damage_mse_mean_per_sensor is not None and damage_losses_per_sensor is not None and \
           ...
           damage_losses_global is not None and damage_ssim_global is not None:
            ...
            logger.info("Generating comparison, statistical, graph heatmap and paper-style plots...") # Mensaje actualizado
