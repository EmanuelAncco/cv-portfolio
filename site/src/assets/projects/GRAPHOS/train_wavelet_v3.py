...
VERSIÓN CON CAPACIDAD MÁXIMA (gnn_hidden=128, rnn_hidden=256) y LR bajo.
"""
...
def define_bridge_graph(num_nodes=5):
    ...
    if edge_index.max() >= num_nodes:
        raise ValueError(f"Índice de nodo {edge_index.max()} en edge_index es inválido para {num_nodes} nodos (índices deben ser de 0 a {num_nodes-1}).")
    return edge_index
...
class SpatioTemporalWaveletDataset(Dataset):
    ...
    def __init__(self, data_dict_features, window_size, stride=1, num_expected_nodes=5):
        ...
        for sid, data in data_dict_features.items():
            ...
            if data is not None and isinstance(data, np.ndarray) and data.ndim == 2 and data.shape[0] >= window_size:
                ...
                if expected_num_features == -1:
                    ...
                    local_logger.error(f"Inconsistencia en número de features. Sensor {sid} tiene {current_num_features}, se esperaban {expected_num_features}. Omitiendo.")
        ...
        for sid in range(1, self.num_expected_nodes + 1):
             if sid in valid_data_dict:
                 ...
                 if self.num_features == 0: # Tomar num_features del primer nodo válido
                  ...
                  local_logger.error(f"Faltan datos procesados para el sensor esperado {sid}. La arquitectura requiere datos de todos los nodos. Abortando.")
        ...
        try:
            ...
            self.data = np.stack(processed_data_list, axis=1)
             ...
             local_logger.error(f"Error al apilar datos (np.stack axis=1). Shapes individuales: [{shapes_str}]. Error: {e}")
             raise e
        ...
        if self.num_nodes != self.num_expected_nodes:
             local_logger.error(f"Inconsistencia crítica: Nodos procesados ({self.num_nodes}) != Nodos esperados ({self.num_expected_nodes}) después del apilado.")
        ...
        if self.num_features != expected_num_features and expected_num_features != -1:
             local_logger.error(f"Inconsistencia crítica: Features procesadas ({self.num_features}) != Features esperadas ({expected_num_features}).")
        ...
        if self.n_samples == 0:
            local_logger.warning(f"Número de muestras es 0. Longitud datos: {self.data.shape[0]}, WinSize: {window_size}, Stride: {stride}.")
        local_logger.info(f"Dataset creado con {self.num_nodes} nodos, {self.num_features} features, {self.data.shape[0]} puntos, {self.n_samples} ventanas.")
    ...
    def __getitem__(self, idx):
        ...
        if start < 0 or end > self.data.shape[0]: # Usar self.data.shape[0]
             local_logger = logging.getLogger(self.__class__.__name__)
             local_logger.error(f"Índice {idx} genera rango [{start}, {end}) fuera de límites [{0}, {self.data.shape[0]}]. Stride={self.stride}, WinSize={self.window_size}, N_Samples={self.n_samples}")
...
class SpatioTemporalAutoencoder(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        if T_actual != self.window_size or N_actual != self.num_nodes or F_actual != self.num_features:
            logger.warning(f"Unexpected input shape in forward: {x.shape}. Expected T={self.window_size}, N={self.num_nodes}, F={self.num_features}. Trying to proceed...")
        ...
        try:
            ...
            logger.error(f"Error en reshape final. Reconstructed frames shape: {reconstructed_frames.shape}. Target: ({batch_size}, {T_actual}, {N_actual}, {F_actual}). Error: {e}", exc_info=True)
            raise e
    ...
    try:
        ...
        for i in range(level, 0, -1):
            ...
            if detail_level_index < 0 or detail_level_index >= len(coeffs):
                 logger.error(f"Índice de nivel de detalle {detail_level_index} fuera de rango para coeffs (len={len(coeffs)}).")
        ...
        if features.shape != (target_len, expected_feature_count):
             logger.warning(f"Shape inesperado en features wavelet: {features.shape}. Esperado: ({target_len}, {expected_feature_count})")
             # Considerar devolver None o lanzar error si el shape es crucial

        return features

    except ValueError as ve: # Errores comunes de PyWavelets
        logger.error(f"Error de valor aplicando DWT (posiblemente nivel muy alto para longitud {original_len}): {ve}", exc_info=True)
        return None
    except Exception as e:
...
def run_experiment_wavelet_gnn(data_directory, output_dir, hp):
    ...
    logger.info(f"Directorio de datos: {data_directory}") # LOG data_directory
    ...
    # Número de features esperado = 1 (original) + 1 (aprox) + wavelet_level (detalles)
    num_expected_features = 1 + 1 + wavelet_level
    logger.info(f"Configuración Wavelet: Name='{wavelet_name}', Level={wavelet_level} -> Features esperadas={num_expected_features}")
    ...
    if not all_files:
        ...
        # Asegurarse de cerrar el handler antes de salir
    ...
    for f_path in tqdm(all_files, desc="Cargando archivos"):
        try:
            ...
            if sid in sensor_data_raw:
                # Leer solo la segunda columna (índice 1) - ASEGURARSE QUE ESTO ES CORRECTO
                data = pd.read_csv(f_path, sep='\s+', header=None, usecols=[1], engine='python', on_bad_lines='warn').values
            ...
            logger.error(f"Error inesperado cargando '{filename}': {e}. Omitiendo.", exc_info=False) # No mostrar traceback completo para cada archivo
            files_skipped += 1
    ...
    for sid, data_list in sensor_data_raw.items():
        if data_list:
            # Filtrar arrays vacíos antes de reshape y concatenar
            ...
            if not data_list_valid:
                ...
                continue
            # Asegurar que todos sean 2D (N, 1)
            data_list_2d = [d.reshape(-1, 1) if d.ndim == 1 else d for d in data_list_valid]
            # Verificar consistencia de columnas (deberían ser 1)
            if any(d.shape[1] != 1 for d in data_list_2d):
                 logger.error(f"Sensor {sid}: Inconsistencia en número de columnas ({[d.shape[1] for d in data_list_2d]}). Omitiendo sensor.")
                 continue
            try:
    ...
    # Verificar si min_len_raw es válido
    ...
    for sid in tqdm(sensors_with_data, desc="Generando Features Wavelet"):
        ...
        # Verificar el número de features generadas
        ...
        if actual_num_features_generated == 0:
            actual_num_features_generated = current_features
             ...
             logger.error(f"Inconsistencia en número de features generadas. Sensor {sid} tiene {current_features}, se esperaban {actual_num_features_generated}.")
             feature_generation_successful = False
             break

        sensor_data_features[sid] = features_2d # Shape: (target_len, actual_num_features_generated)
    ...
    if actual_num_features_generated != num_expected_features:
         logger.warning(f"El número de features wavelet generadas ({actual_num_features_generated}) no coincide con el esperado ({num_expected_features}). Se usará el valor generado: {actual_num_features_generated}.")
         # Actualizar num_expected_features para el resto del script si decidimos continuar
         num_expected_features = actual_num_features_generated

    logger.info(f"Features Wavelet generadas exitosamente. Shape por sensor: ({target_len}, {actual_num_features_generated})")

    # Liberar memoria de datos crudos
    ...
    for sid in sensors_with_data: # Iterar solo sobre los que tienen datos
        try:
            # Asegurar que data tenga el número correcto de features antes de escalar
            ...
            if data_to_scale is None or data_to_scale.shape[1] != actual_num_features_generated:
                logger.error(f"Datos inconsistentes para escalar en sensor {sid}. Shape: {data_to_scale.shape if data_to_scale is not None else 'None'}. Esperado: (*, {actual_num_features_generated})")
                continue # Saltar este sensor
    ...
    try:
        ...
        full_dataset = SpatioTemporalWaveletDataset(
            sensor_data_scaled_features, # Usar el diccionario de datos escalados
            hp['window_size'],
            hp['stride'],
    ...
    if len(full_dataset) == 0:
        logger.error("El dataset de ventanas Wavelet está vacío (n_samples=0). Verifique window_size, stride y longitud de datos.")
        if file_handler: file_handler.close(); logger.removeHandler(file_handler)
        return
    ...
    # Asegurarse de que ambos splits tengan al menos una muestra
    if train_len <= 0 or val_len <= 0:
         logger.error(f"No hay suficientes ventanas para dividir en entrenamiento ({train_len}) y validación ({val_len}) después de crear ventanas. Total: {total_windows}")
         if file_handler: file_handler.close(); logger.removeHandler(file_handler)
         return
    ...
    # *** AJUSTAR BATCH SIZE SI ES NECESSARIO POR MEMORIA ***
    batch_size = hp['batch_size']
    max_retries = 3 # Intentar reducir batch size hasta 3 veces
    current_retry = 0
    ...
    while current_retry <= max_retries:
        try:
            ...
            # Prueba rápida para forzar asignación de memoria (opcional)
            ...
            break # Salir del bucle si funciona
             ...
             if "CUDA out of memory" in str(e) and batch_size > 1:
                 ...
                 batch_size //= 2
                 current_retry += 1
                 # Limpiar memoria GPU antes de reintentar
    ...
    # Actualizar HP con el batch_size final que funcionó
    ...
    # *** Instanciar modelo CON NUEVOS TAMAÑOS de hp ***
    model = SpatioTemporalAutoencoder(
        num_nodes=num_expected_nodes, # Debe ser 5
        num_features=actual_num_features, # El número real generado
        window_size=hp['window_size'],
        gnn_hidden=hp.get('gnn_hidden', 128), # <-- Tomar de HP
        gnn_out=hp.get('gnn_out', 64),       # <-- Tomar de HP
        rnn_hidden=hp.get('rnn_hidden', 256),  # <-- Tomar de HP
        rnn_layers=hp.get('rnn_layers', 2)
    ).to(device)

    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    logger.info(f"Modelo STG-AE (Wavelet Features - Cap Max) creado. Features: {actual_num_features}, Parámetros: {total_params:,}, Batch Size Final: {batch_size}")

    criterion = nn.MSELoss()
    # Usar AdamW puede ser más estable con weight decay
    optimizer = torch.optim.AdamW(model.parameters(), lr=hp['learning_rate'], weight_decay=hp.get('weight_decay', 1e-5)) # <-- Usar AdamW y WD
    ...
    patience_counter = 0
    best_model_path = os.path.join(output_dir, 'best_model_wavelet_gnn.pth') # Nombre consistente
    history = {'train_loss': [], 'val_loss': [], 'lr': []}

    logger.info("\n--- Iniciando Entrenamiento del STG-AE (Wavelet Features - Capacidad Máxima) ---")
    ...
    for epoch in range(hp['epochs']):
        ...
        avg_train_loss = 0.0
        batch_count_train = 0
        progress_bar_train = tqdm(train_loader, desc=f'Epoch {epoch + 1}/{hp["epochs"]} [Train]', leave=False, unit="batch")

        for inputs, _ in progress_bar_train:
            # Validación de shape dentro del bucle por si acaso
            if inputs.shape[1]!=hp['window_size'] or inputs.shape[2]!=num_expected_nodes or inputs.shape[3]!=actual_num_features:
                logger.warning(f"Skip train batch shape {inputs.shape}. Expected (B, {hp['window_size']}, {num_expected_nodes}, {actual_num_features})")
                continue

            inputs = inputs.to(device)
            optimizer.zero_grad(set_to_none=True) # set_to_none=True puede ser más eficiente
            ...
            try:
                ...
                if not torch.isfinite(loss):
                    logger.error(f"Loss NaN/Inf detectada en train epoch {epoch+1}, batch {batch_count_train+1}. Deteniendo entrenamiento.")
                    if file_handler: file_handler.close(); logger.removeHandler(file_handler)
                    return # Detener

                loss.backward()
                # Gradient Clipping sigue siendo importante
            ...
            except RuntimeError as e: # Capturar específicamente OOM en el forward/backward
                 if "CUDA out of memory" in str(e):
                     logger.error(f"CUDA out of memory durante train epoch {epoch+1}, batch {batch_count_train+1}. Batch Size={batch_size}. Intente reducir batch_size aún más.", exc_info=False)
                     # Limpiar memoria e intentar continuar? O abortar? Abortamos por seguridad.
                     if device.type == 'cuda': torch.cuda.empty_cache(); gc.collect()
                     if file_handler: file_handler.close(); logger.removeHandler(file_handler)
                     return # Abortar
                 else:
                     logger.error(f"Error Runtime en train epoch {epoch+1}, batch {batch_count_train+1}: {e}", exc_info=True)
                     continue # Intentar continuar con el siguiente batch
            except Exception as e: # Otros errores
                logger.error(f"Error inesperado en train epoch {epoch+1}, batch {batch_count_train+1}: {e}", exc_info=True)
        ...
        if batch_count_train == 0:
            ...
            # Decidir si continuar o detener. Podría indicar problema con DataLoader. Continuamos por ahora.
            avg_train_loss = 0.0 # O np.nan?
        else:
            avg_train_loss /= batch_count_train
        history['train_loss'].append(avg_train_loss if batch_count_train > 0 else None) # Guardar None si no hubo batches
        ...
        avg_val_loss = 0.0
        batch_count_val = 0
        progress_bar_val = tqdm(val_loader, desc=f'Epoch {epoch + 1}/{hp["epochs"]} [Val]', leave=False, unit="batch")
        ...
        with torch.no_grad():
            for inputs, _ in progress_bar_val:
                ...
                try:
                    ...
                    if not torch.isfinite(loss):
                        logger.warning(f"Loss NaN/Inf detectada en val epoch {epoch+1}, batch {batch_count_val+1}. Omitiendo batch.")
                        continue # No sumar loss inválida

                    avg_val_loss += loss.item()
                    batch_count_val += 1
                    progress_bar_val.set_postfix({'Val Loss': f'{loss.item():.6f}'})
                except RuntimeError as e: # OOM en validación
                    if "CUDA out of memory" in str(e):
                        logger.error(f"CUDA out of memory durante val epoch {epoch+1}. Batch Size={batch_size}. Abortando.", exc_info=False)
                        if device.type == 'cuda': torch.cuda.empty_cache(); gc.collect()
                        if file_handler: file_handler.close(); logger.removeHandler(file_handler)
                        return # Abortar
                    else:
        ...
        if batch_count_val == 0:
            ...
            avg_val_loss = float('inf') # Asignar infinito si no se pudo validar
        else:
            avg_val_loss /= batch_count_val
        history['val_loss'].append(avg_val_loss if batch_count_val > 0 else None) # Guardar None si no hubo batches

        epoch_duration = datetime.now() - epoch_start_time

        # Verificar si avg_val_loss es válido antes de loggear y usar
        # avg_val_loss podría ser inf si batch_count_val fue 0
        if not np.isfinite(avg_val_loss):
             logger.error(f"Epoch {epoch + 1}/{hp['epochs']} -> Train Loss: {avg_train_loss:.6f}, Val Loss: INVALID. Deteniendo.")
             if file_handler: file_handler.close(); logger.removeHandler(file_handler)
             return # Detener
    ...
    logger.info(f"--- Entrenamiento Finalizado (Wavelet GNN - Max Capacity) ---")
    ...
    try:
        with open(hp_path, 'w', encoding='utf-8') as f:
            hp_save = hp.copy()
            hp_save['model_type'] = 'STG-AE (Wavelet Features - Max Capacity)'
    ...
    try:
        ...
        logger.info(f"Historial de pérdidas guardado en: {history_path}")
    except Exception as e:
        logger.error(f"Error guardando historial de pérdidas: {e}")

    # --- Plotear curvas de pérdida ---
    try:
        # Filtrar None antes de plotear
        ...
        # Asegurar que los arrays de épocas coincidan con las losses válidas
        ...
        if not epochs_train or not epochs_val: # Si alguna lista está vacía
            ...
            plt.title('Training & Validation Loss (STG-AE Wavelet - Max Capacity)') # Actualizar título
            plt.xlabel('Epochs')
            plt.ylabel('MSE Loss')

            # Determinar escala Y
            all_losses_plot = train_loss_plot + val_loss_plot
            # Filtrar inf/nan/None antes de min/max
            valid_losses_plot = [l for l in all_losses_plot if l is not None and np.isfinite(l)]
            if not valid_losses_plot:
                 min_loss_plot = 0.001; max_loss_plot = 1.0 # Defaults si no hay datos válidos
            else:
            ...
            if (max_loss_plot / max(min_loss_plot, 1e-9) > 100) or min_loss_plot < 0.01 :
                plt.yscale('log')
                plt.ylabel('MSE Loss (Log Scale)') # Actualizar etiqueta Y
                # Ajustar límite inferior para log, asegurando que no sea cero o negativo
                plot_min_y = max(min_loss_plot * 0.8, 1e-9) # Bajar un poco pero no demasiado
                # Añadir un margen superior también
                ...
                # Escala lineal desde 0 o un poco menos si hay valores negativos (no debería con MSE)
                plt.ylim(bottom=min(0, min_loss_plot * 0.9 if min_loss_plot < 0 else 0), top = max_loss_plot * 1.1)

            plt.legend()
            plt.grid(True, linestyle=':')
            loss_curve_path = os.path.join(output_dir, 'loss_curve_wavelet_gnn_max_cap.png') # Actualizar nombre
            plt.savefig(loss_curve_path, dpi=300)
            plt.close()
            logger.info(f"Gráfico de curvas de pérdida guardado en: {loss_curve_path}")
    ...
    if file_handler:
        ...
        file_handler.close()
        logger.removeHandler(file_handler) # Importante removerlo
...
if __name__ == '__main__':
    ...
    # !!! ASEGÚRATE QUE ESTA RUTA EXISTE Y TIENES PERMISOS DE ESCRITURA !!!
    base_output_dir = r"D:\Python_proyectos_2025\GAIATECH\resultados_entrenamiento_wavelet" # Mismo directorio base

    # --- Hiperparámetros (CON CAPACIDAD MÁXIMA) ---
    HP = {
        # Dataset & Wavelet
        "window_size": 64, "stride": 32, "wavelet_name": "db4", "wavelet_level": 5,
        # *** Modelo (Capacidad Máxima) ***
        "gnn_hidden": 128,      # <-- Duplicado de nuevo
        "gnn_out": 64,          # <-- Duplicado de nuevo
        "rnn_hidden": 256,      # <-- Duplicado de nuevo
        "rnn_layers": 2,
        # Entrenamiento
        "epochs": 50,           # Mantener 50 épocas por ahora
        "batch_size": 16,       # *** REDUCIDO batch_size PREVENTIVAMENTE por memoria ***
        "learning_rate": 0.0005,# *** EMPEZAR con LR más bajo ***
        "patience": 10,
        "scheduler_patience": 5,
        "scheduler_factor": 0.5,
        "weight_decay": 1e-5    # *** AÑADIDO weight decay ***
    }

    # --- Crear Directorio de Salida ---
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    # Añadir info de capacidad al nombre
    output_directory = os.path.join(base_output_dir, f"run_wavelet_{HP['wavelet_name']}{HP['wavelet_level']}_h{HP['gnn_hidden']}_r{HP['rnn_hidden']}_lr{HP['learning_rate']}_wd{HP['weight_decay']}_{timestamp}") # Nombre más descriptivo
    try:
        os.makedirs(output_directory, exist_ok=True)
        # Usar print aquí porque el logger de archivo aún no está configurado
    ...
    try:
        run_experiment_wavelet_gnn(data_folder_path, output_dir=output_directory, hp=HP) # Pasar output_directory
    except Exception as e:
        # Loggear la excepción si el logger ya está configurado, sino imprimir
        # Comprobar si el handler de archivo fue añadido antes de intentar loggear
        ...
        # Intentar cerrar el handler si existe, aunque haya habido error
        for handler in logger.handlers[:]:
             if isinstance(handler, logging.FileHandler):
                 try: handler.close(); logger.removeHandler(handler)
                 except: pass # Ignorar errores al cerrar/remover
