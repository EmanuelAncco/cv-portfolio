...
y guarda el modelo final en formato SavedModel.
...
# Nombre del archivo donde se guardará el mejor checkpoint durante el entrenamiento
CHECKPOINT_PATH = "vit_checkpoint.keras"
# Nombre de la carpeta donde se guardará el modelo final en formato SavedModel
FINAL_MODEL_PATH = "modeloGAIA" # Nombre solicitado para el modelo final
# ────────────────────────────────────────────────────────────────────────────
...
def compile_and_train_vit(model, train_ds, val_ds, class_weight,
                          ...
                          checkpoint=CHECKPOINT_PATH): # Usar la constante CHECKPOINT_PATH
...
def evaluate_and_save_vit(best_path, val_ds, num_classes, final_path=FINAL_MODEL_PATH): # Usar la constante FINAL_MODEL_PATH
    ...
    try:
        # Guardamos en la ruta final especificada
...
# La función convert_to_tflite ya no se llama en main, pero la mantenemos por si acaso.
...
def main():
    ...
    hist, best_ckpt = compile_and_train_vit(model, train_ds, val_ds, class_weight,
                                            epochs=EPOCHS_FINE_TUNE, lr=LEARNING_RATE,
                                            checkpoint=CHECKPOINT_PATH) # Usar la constante

    # Evaluar el mejor modelo y guardarlo en formato SavedModel
    # Guardamos en la ruta final especificada (modeloGAIA)
    final_model = evaluate_and_save_vit(best_ckpt, val_ds, num_classes=num_classes, final_path=FINAL_MODEL_PATH)

    # --- Omitimos la conversión a TFLite según la solicitud ---
