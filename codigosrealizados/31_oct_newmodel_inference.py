...
4. Generar gráficas de la arquitectura (Grafo) y pre-procesamiento (Wavelets).
5. NUEVO: Graficar el error de reconstrucción a lo largo del tiempo.
...
def load_raw_data(data_dir, num_expected_nodes=5):
    ...
    for f_path in all_files:
        try:
            ...
            logger.warning(
                f"No se pudo procesar el archivo {f_path}, omitiendo.")
            pass
    ...
    if min_len_raw == float('inf'):
        logger.error(f"min_len_raw sigue siendo infinito, no se cargaron datos.")
        return None, 0
...
def process_data_with_scaler(sensor_data_concat, min_len, hp, scaler):
    ...
    for sid in range(1, num_expected_nodes + 1):
        if sid not in sensor_data_concat:
            logger.error(f"Faltan datos concatenados para S{sid} en process_data_with_scaler")
            return None
...
def get_reconstruction_errors_per_sensor(model, dataloader, device, edge_index, edge_weight):
    ...
    with torch.no_grad():
        for (inputs, _) in tqdm(dataloader, desc=desc):
            ...
            errors_per_sensor_list.append(error_per_sensor.cpu().numpy())

    # Retorna (N_samples, N_sensores)
    return np.array(errors_per_sensor_list)
...
def plot_error_over_time(errors_dict, hp, output_dir):
    ...
    if healthy_errors is None or damage_errors is None:
        logger.warning("No se pueden graficar errores en el tiempo sin datos 'Sano' y 'Daño Real'.")
        return
    ...
    axs[0].set_title('Error de Reconstrucción en el Tiempo (Datos Sanos)')
    ...
    axs[1].axhline(threshold, color='r', linestyle='--',
                   label=f'Umbral ({threshold:.4f})')
    axs[1].set_title('Error de Reconstrucción en el Tiempo (Datos Daño Real)')
...
def run_evaluation(run_dir, healthy_data_dir, damage_data_dir):
    ...
    healthy_loader = DataLoader(val_dataset, batch_size=1, shuffle=False)
    logger.info(
    ...
    damage_loader = DataLoader(damage_dataset, batch_size=1, shuffle=False)
    logger.info(
