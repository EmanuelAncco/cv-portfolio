...
# (Según tus preferencias de proyecto, para trazabilidad científica)
logging.basicConfig(
    level=logging.INFO,
...
def calculate_single_point_fs(params):
    ...
    try:
        ...
        epsilon_p_ax = epsilon_g_ax * (1 / (1 + termino_rigidez))  # Ec. 10
...
class AdvancedPipelineAnalysisApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Simulador de Tesis Avanzado - E. Ancco (TGD Probabilístico)")
        ...
        main_paned_window.add(control_frame, weight=1)
        ...
        self.param_labels = {}
        # --- FIX: Usar enumerate para la fila (row) en lugar de index() ---
        param_keys_to_display = ["PGV", "Vs", "Es", "Kt", "Sy"]
        ...
        main_paned_window.add(results_frame, weight=3)
        ...
        self.notebook.add(self.tab_summary, text="Resultados de Confiabilidad")
        ...
        self.notebook.add(self.tab_sens_det, text="Sensibilidad Determinista (Gráficos)")
        self.fig_sens_det, (self.ax_sens_theta, self.ax_sens_vs) = plt.subplots(2, 1, figsize=(7, 8),
        ...
        self.plot_deterministic_sensitivity()  # Plot estático al inicio

        # Pestaña 5: Sensibilidad Probabilística (Tornado)
        self.tab_sens_prob = ttk.Frame(self.notebook, padding=10)
        self.notebook.add(self.tab_sens_prob, text="Sensibilidad Probabilística (Tornado)")
    ...
    def plot_deterministic_sensitivity(self):
        ...
        self.ax_sens_theta.clear()
        self.ax_sens_vs.clear()

        # --- Gráfico 1: FS vs. Ángulo (theta) ---
        for scenario_name, params in SCENARIOS.items():
            ...
            self.ax_sens_theta.plot(thetas, fss, 'o-', label=scenario_name)

        self.ax_sens_theta.set_title("Sensibilidad Determinista: FS vs. Ángulo de Incidencia ($\theta$)")
        self.ax_sens_theta.set_xlabel("Ángulo de Incidencia $\\theta$ (°)")
        self.ax_sens_theta.set_ylabel("Factor de Seguridad (FS)")
        self.ax_sens_theta.legend()
        self.ax_sens_theta.grid(True, linestyle=':', alpha=0.6)
        self.ax_sens_theta.axhline(1.0, color='red', linestyle='--', label="FS = 1.0 (Falla)")

        # --- Gráfico 2: FS vs. Velocidad de Onda (Vs) ---
        # (Basado en el Caso Surco, variando Vs)
        base_params = {
        ...
        for vs in vs_range:
            params = base_params.copy()
            params["Vs"] = vs
            # Asumimos que Es se correlaciona con Vs (simplificación)
            params["Es"] = (vs / 400.0) * SCENARIOS["Caso Surco (Tipo C)"]["Es"]["mean"]
            res = calculate_single_point_fs(params)
            fs_vs_results.append(res['FS'])

        self.ax_sens_vs.plot(vs_range, fs_vs_results, 'o-')
        self.ax_sens_vs.set_title("Sensibilidad Determinista: FS vs. Velocidad de Onda ($V_s$)")
        self.ax_sens_vs.set_xlabel("Velocidad de Onda de Corte $V_s$ (m/s)")
        self.ax_sens_vs.set_ylabel("Factor de Seguridad (FS)")
        self.ax_sens_vs.grid(True, linestyle=':', alpha=0.6)
        self.ax_sens_vs.axhline(1.0, color='red', linestyle='--', label="FS = 1.0 (Falla)")
    ...
    def export_to_csv(self):
        ...
        try:
            ...
            df = pd.DataFrame({
                "Resistencia (R)": self.R_results,
                "Solicitacion (S)": self.S_results
            })
