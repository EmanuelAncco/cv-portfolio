...
def calculate_detailed_stress(params):
    ...
    # --- NUEVO: Cálculo de Deformación ---
    # Se calcula la deformación radial externa basada en el estado de esfuerzo biaxial
    # epsilon_h = (1/E) * (sigma_h - nu * sigma_L) -> Deformación unitaria circunferencial
    # u_r = R * epsilon_h -> Desplazamiento radial
    epsilon_h_ext = (1 / E_ACERO) * (sigma_h_total_ext - NU_ACERO * sigma_L_total_ext)
    u_r_ext = R * epsilon_h_ext
    ...
    results = {"theta_deg": np.rad2deg(theta), "M_total": M_total, "N_total": N_total,
               ...
               # Nuevos resultados para el gráfico de deformación
               "u_r_ext": u_r_ext, "R_mean": R
               }
    return results
...
class PipelineStressApp:
    ...
    def setup_styles(self):
        ...
        style.configure("TRadiobutton", background='#ffffff', font=('Arial', 9))
    ...
    def setup_single_point_tab(self):
        ...
        # Variable para controlar el tipo de gráfico a mostrar en esta pestaña
        self.sp_plot_type = tk.StringVar(value='stress')
        ...
        def add_row(key, label, unit):
            ...
            if key == "district_geo":
                ...
                self.add_editable_combobox(frame, row, "sp_", "D", "Diámetro Ext. D", list(COMMON_DIAMETERS.keys()),
                                           self.sp_on_diameter_selected, "m");
                row += 1
            elif key == "t":
                self.add_editable_combobox(frame, row, "sp_", "t", "Espesor t", list(COMMON_THICKNESSES.keys()),
                                           self.sp_on_thickness_selected, "m");
                row += 1
            elif "display" in key:
                self.add_readonly_entry(frame, row, "sp_", key, label, unit);
                row += 1
            else:
                self.add_entry(frame, row, "sp_", key, label, unit);
                row += 1
    ...
    def create_sp_results_panel(self, parent):
        ...
        # NUEVO: Controles para seleccionar el tipo de gráfico
        sp_plot_controls = ttk.Frame(frame, style="Right.TFrame")
        sp_plot_controls.grid(row=1, column=0, pady=5)
        ttk.Radiobutton(sp_plot_controls, text="Gráfico de Esfuerzo", variable=self.sp_plot_type, value='stress',
                        command=self.sp_update_plot).pack(side=tk.LEFT, padx=10)
        ttk.Radiobutton(sp_plot_controls, text="Gráfico de Deformación", variable=self.sp_plot_type,
                        value='deformation', command=self.sp_update_plot).pack(side=tk.LEFT, padx=10)

        self.sp_plot_container = ttk.Frame(frame);
        self.sp_plot_container.grid(row=2, column=0, sticky="nsew")
        return frame
    ...
    def sp_update_plot(self):
        ...
        if hasattr(self, 'sp_fig_canvas') and self.sp_fig_canvas:
            self.sp_fig_canvas.get_tk_widget().destroy()

        results = self.sp_last_results
        Sy = results['Sy']

        # Actualizar la etiqueta de estado (común para ambos gráficos)
        ...
        # Crear el nuevo gráfico según la selección
        fig, ax = plt.subplots(figsize=(6, 6), subplot_kw={'projection': 'polar'})
        plot_type = self.sp_plot_type.get()
        ...
        if plot_type == 'stress':
            ...
            R = results.get("R_mean", 0)
            u_r = results.get("u_r_ext", np.zeros_like(results["theta_deg"]))

            # Determinar un factor de escala para que la deformación sea visible
            max_deformation = np.max(np.abs(u_r))
            if max_deformation > 1e-9:  # Evitar división por cero
                # Escalar la deformación para que sea ~10% del radio en la visualización
                scale_factor = (R * 0.1) / max_deformation
            else:
                scale_factor = 1.0
            ...
            # Dibujar la forma original (círculo)
            original_radius = np.full_like(full_theta, R)
            ax.plot(full_theta, original_radius, 'k--', label=f'Forma Original (R={R * 1000:.1f} mm)')

            # Dibujar la forma deformada y escalada
            full_u_r = np.concatenate([u_r, np.flip(u_r[1:-1])])
            deformed_radius = R + full_u_r * scale_factor
            ax.plot(full_theta, deformed_radius, 'b-', label=f'Forma Deformada (x{scale_factor:.0f})')

            ax.set_title(f'Deformación Radial (Escalada x{scale_factor:.0f})', pad=20, fontsize=12)
            ax.set_yticklabels([])  # Ocultar etiquetas radiales porque la escala es engañosa
            ax.legend(loc='lower right')

        # Renderizar el gráfico en la GUI
        ...
        def add_row(key, label, unit):
            ...
            if key == "district_geo":
                ...
                self.add_editable_combobox(props_frame, row, "", "D", "Diámetro Ext. D", list(COMMON_DIAMETERS.keys()),
                                           self.on_diameter_selected, "m");
                row += 1
            elif key == "t":
                self.add_editable_combobox(props_frame, row, "", "t", "Espesor t", list(COMMON_THICKNESSES.keys()),
                                           self.on_thickness_selected, "m");
                row += 1
            elif "display" in key:
                self.add_readonly_entry(props_frame, row, "", key, label, unit);
                row += 1
            else:
                self.add_entry(props_frame, row, "", key, label, unit);
                row += 1
    ...
    def setup_right_panel(self, parent):
        ...
        ttk.Button(plot_2d_controls_frame, text="Polar Esfuerzo", command=lambda: self.set_plot_type('polar')).pack(
            fill="x", padx=2)
        ttk.Button(plot_2d_controls_frame, text="Polar Deformación",
                   command=lambda: self.set_plot_type('deformation')).pack(fill="x", padx=2, pady=5)
    ...
    def update_2d_plot(self):
        ...
        if self.current_plot_type == 'superimposed':
            ...
            if self.current_plot_type == 'polar':
            ...
            elif self.current_plot_type == 'deformation':
                ax = fig.add_subplot(111, polar=True)
                R = results.get("R_mean", 0)
                u_r = results.get("u_r_ext", np.zeros_like(results["theta_deg"]))

                max_deformation = np.max(np.abs(u_r))
                if max_deformation > 1e-9:
                    scale_factor = (R * 0.1) / max_deformation
                else:
                    scale_factor = 1.0
                ...
                ax.set_title(f'Deformación Radial (Escalada x{scale_factor:.0f})', pad=20, fontsize=10)
                ax.set_yticklabels([])
