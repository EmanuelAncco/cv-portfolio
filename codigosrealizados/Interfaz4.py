...
Permite agregar más imágenes y analizar solo las nuevas.
Usa el modelo ResNet50V2.
v13: Añade botón "Analizar Nuevas".
"""
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("Clasificador de Enfermedades de Papa v8.0") # Versión actualizada
        ...
        BTN_STOP_COLOR = '#e74c3c'
        BTN_STOP_ACTIVE_COLOR = '#c0392b'
        BTN_ADD_COLOR = '#2ecc71'
        BTN_ADD_ACTIVE_COLOR = '#27ae60'
        BTN_RUN_NEW_COLOR = '#f39c12' # Naranja para Analizar Nuevas
        BTN_RUN_NEW_ACTIVE_COLOR = '#f1c40f'
        ...
        self.style.configure("Add.TButton", padding=8, relief="flat", font=('Segoe UI', 10, 'bold'), background=BTN_ADD_COLOR, foreground="white")
        self.style.map("Add.TButton", background=[('active', BTN_ADD_ACTIVE_COLOR)])
        # Estilo Botón Analizar Nuevas
        self.style.configure("RunNew.TButton", padding=8, relief="flat", font=('Segoe UI', 10, 'bold'), background=BTN_RUN_NEW_COLOR, foreground="white")
        self.style.map("RunNew.TButton", background=[('active', BTN_RUN_NEW_ACTIVE_COLOR)])
        ...
        self.last_processed_index = -1 # Índice del último archivo procesado
        ...
        left_frame.rowconfigure(2, weight=1) # Hacer que el listbox se expanda

        # Frame para botones de selección
        select_button_frame = ttk.Frame(left_frame)
        select_button_frame.grid(row=0, column=0, columnspan=2, sticky="ew", pady=(5, 10))
        select_button_frame.columnconfigure(0, weight=1)
        select_button_frame.columnconfigure(1, weight=1)

        self.select_button = ttk.Button(select_button_frame, text="Nueva Selección...", command=self.select_files)
        self.select_button.grid(row=0, column=0, sticky="ew", padx=(0, 2))

        self.add_button = ttk.Button(select_button_frame, text="Agregar Imágenes...", command=self.add_files, style="Add.TButton")
        self.add_button.grid(row=0, column=1, sticky="ew", padx=(2, 0))
        ...
        self.file_listbox.grid(row=1, column=0, sticky="nsew", padx=5, pady=5) # Movido a fila 1
        list_scrollbar = ttk.Scrollbar(left_frame, orient='vertical', command=self.file_listbox.yview)
        list_scrollbar.grid(row=1, column=1, sticky='ns') # Movido a fila 1
        self.file_listbox['yscrollcommand'] = list_scrollbar.set

        # Frame para botones de acción
        action_button_frame = ttk.Frame(left_frame)
        action_button_frame.grid(row=2, column=0, columnspan=2, sticky="ew", pady=(10, 5)) # Movido a fila 2
        action_button_frame.columnconfigure(0, weight=1)
        action_button_frame.columnconfigure(1, weight=1)
        action_button_frame.columnconfigure(2, weight=1) # Añadir columna para tercer botón

        self.run_all_button = ttk.Button(action_button_frame, text="Analizar Todo", command=lambda: self.run_predictions(analyze_all=True), state=tk.DISABLED)
        self.run_all_button.grid(row=0, column=0, sticky="ew", padx=(0, 2))

        self.run_new_button = ttk.Button(action_button_frame, text="Analizar Nuevas", command=lambda: self.run_predictions(analyze_all=False), state=tk.DISABLED, style="RunNew.TButton")
        self.run_new_button.grid(row=0, column=1, sticky="ew", padx=2)
        ...
        self.stop_button.grid(row=0, column=2, sticky="ew", padx=(2, 0))


        self.progress_bar = ttk.Progressbar(left_frame, orient='horizontal', mode='determinate')
        self.progress_bar.grid(row=3, column=0, columnspan=2, sticky="ew", padx=5, pady=(5, 5)) # Movido a fila 3
        ...
        category_titles = { CLASS_NAMES[0]: "Tizón Temprano (Predicho)", CLASS_NAMES[1]: "Tizón Tardío (Predicho)", CLASS_NAMES[2]: "Sana (Predicho)" }
        for i, class_name in enumerate(CLASS_NAMES):
            ...
            scrollbar_y = ttk.Scrollbar(cat_frame, orient='vertical', command=canvas.yview); scrollbar_y.grid(row=0, column=1, sticky='ns')
            scrollbar_x = ttk.Scrollbar(cat_frame, orient='horizontal', command=canvas.xview); scrollbar_x.grid(row=1, column=0, sticky='ew')
            canvas.configure(yscrollcommand=scrollbar_y.set, xscrollcommand=scrollbar_x.set)
            inner_frame = ttk.Frame(canvas); canvas.create_window((0, 0), window=inner_frame, anchor='nw')
        ...
        # (Sin cambios en esta sección)
        right_frame = ttk.Frame(main_frame); right_frame.grid(row=0, column=2, sticky="nsew", padx=(10, 0), pady=5)
        right_frame.rowconfigure(1, weight=1); right_frame.rowconfigure(2, weight=2); right_frame.columnconfigure(0, weight=1)
        rec_button_frame = ttk.LabelFrame(right_frame, text="3. Ver Recomendaciones", padding="10"); rec_button_frame.grid(row=0, column=0, sticky="ew", pady=(0, 5))
        ...
        self.solutions_text = scrolledtext.ScrolledText(right_frame, wrap=tk.WORD, height=10, state=tk.DISABLED, font=('Segoe UI', 9), bg="white", fg=self.FG_COLOR, relief="flat", borderwidth=1); self.solutions_text.grid(row=1, column=0, sticky="nsew", pady=(0, 10))
        history_frame = ttk.LabelFrame(right_frame, text="4. Historial", padding="10"); history_frame.grid(row=2, column=0, sticky="nsew")
        history_frame.rowconfigure(0, weight=1); history_frame.columnconfigure(0, weight=1)
        self.history_tree = ttk.Treeview(history_frame, columns=('#', 'Archivo', 'Clase', 'Confianza'), show='headings'); self.history_tree.heading('#', text='#'); self.history_tree.heading('Archivo', text='Archivo'); self.history_tree.heading('Clase', text='Clase Predicha'); self.history_tree.heading('Confianza', text='Confianza (%)')
    ...
    def select_files(self):
        """Inicia una NUEVA selección, limpiando resultados anteriores."""
        filetypes = (('Imágenes', '*.jpg *.jpeg *.png *.bmp *.gif'), ('Todos los archivos', '*.*'))
        filenames = filedialog.askopenfilenames(title='Nueva Selección de Imágenes', filetypes=filetypes)
        if filenames:
            self.selected_files = list(filenames)
            self.update_file_list()
            self.update_run_buttons_state() # Actualizar estado de botones
            self.clear_displays() # Limpia todo para la nueva selección
        else:
            if not self.selected_files:
                 self.update_run_buttons_state()

    def add_files(self):
        """AGREGA imágenes a la selección existente."""
        # No actuar si el proceso está corriendo
        if self.run_all_button['state'] == tk.DISABLED and self.stop_button['state'] == tk.NORMAL:
            messagebox.showwarning("Procesando", "No se pueden agregar imágenes mientras se procesa.")
            return

        filetypes = (('Imágenes', '*.jpg *.jpeg *.png *.bmp *.gif'), ('Todos los archivos', '*.*'))
        filenames = filedialog.askopenfilenames(title='Agregar Imágenes a la Selección', filetypes=filetypes)
        if filenames:
            new_files_added = 0
            for f in filenames:
            ...
            if new_files_added > 0:
                self.update_file_list()
                self.update_run_buttons_state() # Actualizar estado de botones
                print(f"{new_files_added} imágenes agregadas a la lista.")
            else:
                print("No se agregaron nuevas imágenes (posiblemente duplicados).")

    def update_file_list(self):
        self.file_listbox.delete(0, tk.END)
        if self.selected_files:
            for i, filepath in enumerate(self.selected_files):
                prefix = "[NUEVO] " if i > self.last_processed_index else ""
                self.file_listbox.insert(tk.END, f"{prefix}{i+1}. {os.path.basename(filepath)}")
                # Colorear si es nuevo (opcional, puede ser lento)
                # if i > self.last_processed_index:
                #     self.file_listbox.itemconfig(tk.END, {'fg': 'blue'})
        else:
            self.file_listbox.insert(tk.END, "(Ningún archivo seleccionado)")

    def add_image_to_category_grid(self, img_pil, category, global_number):
        ...
        try:
            ...
            num_images_in_cat = sum(1 for w in inner_frame.winfo_children() if isinstance(w, ttk.Frame))
    ...
    def clear_displays(self):
         """Limpia grids, historial, soluciones y resetea contadores."""
         ...
         self.processed_counter = 0
         self.last_processed_index = -1 # Resetear índice al limpiar
    ...
    def update_run_buttons_state(self):
        """Actualiza el estado de los botones 'Analizar'."""
        if self.selected_files:
            self.run_all_button.config(state=tk.NORMAL)
            # Habilitar "Analizar Nuevas" solo si hay archivos pendientes
            if self.last_processed_index < len(self.selected_files) - 1:
                self.run_new_button.config(state=tk.NORMAL)
            else:
                self.run_new_button.config(state=tk.DISABLED)
        else:
            self.run_all_button.config(state=tk.DISABLED)
            self.run_new_button.config(state=tk.DISABLED)


    def run_predictions(self, analyze_all=True):
        """Ejecuta predicciones sobre archivos seleccionados."""
        if not self.selected_files:
        ...
        if analyze_all:
            ...
            self.clear_displays()
            self.update_solutions_text("Procesando todas las imágenes...")
        else:
            # Empezar desde el siguiente al último procesado
            start_index = self.last_processed_index + 1
            if start_index >= len(self.selected_files):
                messagebox.showinfo("Nada Nuevo", "No hay nuevas imágenes para analizar.")
                return
            self.update_solutions_text(f"Procesando imágenes nuevas desde el índice {start_index+1}...")

        self.model = load_model_lazily()
        if self.model is None: return

        self.stop_processing = False
        self.progress_bar['value'] = start_index # Iniciar barra de progreso desde donde corresponde
        self.progress_bar['maximum'] = len(self.selected_files)
        self.root.update_idletasks()

        # Deshabilitar botones de selección y análisis, habilitar detener
        ...
        current_processed_count = 0 # Contador para este ciclo específico

        # Iterar solo sobre los archivos necesarios
        files_to_process = self.selected_files[start_index:]
        for i, filepath in enumerate(files_to_process):
            current_index_in_list = start_index + i # Índice real en self.selected_files

            if self.stop_processing:
                print("Proceso detenido por el usuario.")
                self.update_solutions_text("Proceso detenido por el usuario.")
                self.last_processed_index = current_index_in_list - 1 # Guardar el último completado
                break

            self.processed_counter += 1 # Incrementar contador global
            current_processed_count += 1 # Incrementar contador de este ciclo
            filename = os.path.basename(filepath)
            print(f"Procesando #{self.processed_counter} (Índice lista: {current_index_in_list}): {filename}")

            pred_class, pred_conf, raw_scores, img_for_thumb = predict_single_image(self.model, filepath)

            if img_for_thumb:
                self.add_image_to_category_grid(img_for_thumb, pred_class, self.processed_counter)
                last_pred_class = pred_class

            self.add_to_history(self.processed_counter, filename, pred_class, pred_conf)

            self.progress_bar['value'] = current_index_in_list + 1 # Actualizar barra con índice real
            if (i + 1) % 5 == 0 or (i + 1) == len(files_to_process):
                 self.root.update_idletasks()

            # Actualizar el índice del último procesado *después* de procesarlo
            self.last_processed_index = current_index_in_list

        # Mensaje final
        if self.stop_processing:
             messagebox.showinfo("Detenido", f"Proceso detenido. Se analizaron {current_processed_count-1} imágenes en esta tanda.")
        else:
            self.update_solutions_text("Proceso completado. Haz clic en un botón de categoría para ver recomendaciones.")
            messagebox.showinfo("Completado", f"Se procesaron {current_processed_count} imágenes.")

        # Resetear estado de botones
        self.select_button.config(state=tk.NORMAL)
        self.add_button.config(state=tk.NORMAL)
        self.update_run_buttons_state() # Actualizar estado de botones de análisis
