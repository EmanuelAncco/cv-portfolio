...
# *** NUEVO: Ruta actualizada para los datos ANÓMALOS (sismo). ***
...
if os.path.isdir(ANOMALY_DATA_DIR):
    ...
    anomaly_errors = []
    anomaly_data_samples = []  # Para guardar muestras para graficar
    ...
    if not anomaly_files:
        ...
        with torch.no_grad():
            for filename in anomaly_files:
                ...
                # Usar el MISMO scaler entrenado con los datos normales. ¡No re-entrenar!
                ...
                if len(anomaly_dataset) > 0 and len(anomaly_data_samples) < 3:
                    # Guardar la primera ventana de este archivo para la visualización
        ...
        # --- Gráfico de Histograma Comparativo de Errores ---
        plt.figure(figsize=(12, 7))
        # Usar log=True para ver mejor la separación si las escalas son muy diferentes
        plt.hist(results_df['error'], bins=50, alpha=0.7, color='steelblue', label='Normal Data Errors', density=True)
        plt.hist(anomaly_errors, bins=50, alpha=0.7, color='darkorange', label='Anomaly (Sismo) Errors', density=True)
        ...
        if anomaly_data_samples:
            # *** MODIFICADO: Ahora crea una figura con 2 filas por muestra para el error ***
            ...
            for i, sample in enumerate(anomaly_data_samples):
                ...
                # Gráfico 1: Comparación de señales
                ax1 = axes[i, 0]
