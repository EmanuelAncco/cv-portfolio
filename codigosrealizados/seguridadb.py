...
class EMARCInferenceApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title("Analizador de Inferencia - EMARC VISIÓN v1.2")
    ...
    def load_model(self):
        if self.model_path and os.path.exists(self.model_path):
            ...
            logging.error("El archivo del modelo 'best.pt' no fue encontrado.")
            messagebox.showerror("Error de Modelo",
                                 "No se encontró el archivo 'best.pt'.\nAsegúrate de que el entrenamiento se completó.")
            self.destroy()
    ...
    def stop_video(self):
        ...
        if self.video_thread and self.video_thread.is_alive():
            # Esperar un poco a que el bucle termine
            time.sleep(0.1)
        ...
        try:
            while self.current_filepath and self.video_capture and self.video_capture.isOpened():
                if self.is_playing:
                    ...
                    self.update_results_summary(results[0])
                # Pequeña pausa para no sobrecargar la CPU y permitir que la GUI responda
                time.sleep(0.01)
        except Exception as e:
            logging.error(f"Error en el bucle de video: {e}")

    def display_image(self, cv2_image):
        # Convertir imagen de OpenCV (BGR) a formato Pillow (RGB)
