        ...
        with torch.no_grad():
            for inputs in test_dataloader:
                ...
                for i in range(inputs.size(0)):
                    ...
                    reconstructed = outputs[i].squeeze().cpu().numpy()
...
if os.path.exists(HISTORY_FILE):
    ...
    # *** NUEVO: Gráfico de entrenamiento con dos subplots para Pérdida y Precisión (SSIM) ***
...
# *** NUEVO: Histograma de Errores de Reconstrucción ***
plt.figure(figsize=(10, 6))
plt.hist(results_df['error'], bins=50, alpha=0.7, color='steelblue')
plt.title('Distribution of Reconstruction Errors')
plt.xlabel('Reconstruction Error (MSE)')
plt.ylabel('Frequency')
# Trazar una línea para el umbral de anomalía (percentil 98) como ejemplo
anomaly_threshold = results_df['error'].quantile(0.98)
plt.axvline(anomaly_threshold, color='red', linestyle='--',
            label=f'Anomaly Threshold (98th percentile = {anomaly_threshold:.4e})')
plt.legend()
plt.grid(True)
plt.show()
...
if len(test_dataset) > 0:
    ...
    with torch.no_grad():
        ...
        if num_samples_to_plot == 1: axes = [axes]
