...
# Definir una paleta de colores para los sensores
sensor_colors = plt.cm.tab10(np.linspace(0, 1, 10))  # Usar tab10 para colores distintos


def plot_reconstruction_sample(original_scaled, reconstructed_scaled, scaler, window_idx, output_dir, prefix):
    """
    ...
    if original_scaled.ndim != 3 or reconstructed_scaled.ndim != 3 or original_scaled.shape != reconstructed_scaled.shape:
        local_logger_plot.error(
            f"Shapes inconsistentes para ploteo: Orig {original_scaled.shape}, Rec {reconstructed_scaled.shape}")
        return
    if original_scaled.shape[2] != 1:  # Asumiendo F=1
        local_logger_plot.error(f"Se esperaba 1 característica, pero se encontraron {original_scaled.shape[2]}")
        return
    ...
    try:
        ...
        local_logger_plot.error(f"Error durante inverse_transform en ploteo: {e}. Graficando datos escalados.")
    ...
    for i in range(num_sensors):
        color = sensor_colors[i % len(sensor_colors)]  # Asignar color al sensor
        ax_sig = axes[i, 0]
        ax_err = axes[i, 1]

        # Usar el color asignado
        ax_sig.plot(original_inv[:, i, 0], label='Original', color=color, linewidth=1.5, alpha=0.8)
        ax_sig.plot(reconstructed_inv[:, i, 0], label='Reconstruida', color=color, linestyle='--', linewidth=1.5,
                    alpha=1.0)
        ax_sig.set_title(f'Sensor {i + 1}: Señal Original vs. Reconstruida')
        ax_sig.set_ylabel('Valor Original (Invertido)')
        ...
        ax_err.set_ylabel('Error (Unidades Originales)')
...
def plot_mse_comparison(mse_healthy_per_sensor_avg, mse_damage_per_sensor_avg, output_dir):
    ...
    if valid_damage_mse.size != num_sensors:
        local_logger_plot.warning(
            "Errores MSE de daño no positivos encontrados para algunos sensores. Usando escala lineal.")
        use_log_scale = False
...
def plot_damage_localization(losses_damage_per_sensor, losses_healthy_per_sensor, output_dir):
    ...
    if not valid_data_indices:
        ...
        for patch, i in zip(bp['boxes'], valid_data_indices):
            patch.set_facecolor(sensor_colors[i % len(sensor_colors)])  # Color por sensor
            patch.set_alpha(0.7)
    ...
    # Usar colores por sensor tambien en barras
    bar_colors = [sensor_colors[i % len(sensor_colors)] for i in range(num_sensors)]
    axes[1].bar(sensor_labels, amplification_factor, color=bar_colors, edgecolor='black', alpha=0.8)
...
def plot_error_distribution_kde(losses_healthy, losses_damage, output_dir):
    ...
    if losses_healthy is None or losses_damage is None or losses_healthy.size == 0 or losses_damage.size == 0:
        local_logger_plot.warning("Datos de pérdidas (healthy o damage) vacíos, omitiendo gráfico KDE.")
        return

    local_logger_plot.info("Generando gráfico de distribución de errores (KDE)...")
    ...
    if len(log_losses_healthy) > 1:  # KDE necesita al menos 2 puntos
        sns.kdeplot(log_losses_healthy, label='Sano (Healthy)', color='blue', fill=True, bw_adjust=0.5)
    else:
        local_logger_plot.warning("No hay suficientes datos sanos positivos para graficar KDE.")

    if len(log_losses_damage) > 1:  # KDE necesita al menos 2 puntos
        sns.kdeplot(log_losses_damage, label='Daño (Damage)', color='red', fill=True, bw_adjust=0.5)
    else:
        local_logger_plot.warning("No hay suficientes datos de daño positivos para graficar KDE.")

    plt.title('Distribución del Error de Reconstrucción (MSE por Ventana)')
    plt.xlabel('Log10(Error Cuadrático Medio)')
    plt.ylabel('Densidad')
    plt.legend()
    ...
    try:
        ...
        local_logger_plot.error(f"Error al guardar el gráfico KDE {filename}: {e}")
    plt.close()
...
def plot_error_timeseries(losses_healthy, losses_damage, output_dir):
    ...
    losses_healthy, losses_damage: shape (n_samples,)
    """
    local_logger_plot.info("Generando gráfico de series de tiempo del error...")
    fig, axes = plt.subplots(2, 1, figsize=(15, 10), sharex=True)
    fig.suptitle('Error de Reconstrucción (MSE) a lo largo del Tiempo (Ventanas)', fontsize=16)
    ...
    if losses_healthy is not None and losses_healthy.size > 0:
        ...
        if len(valid_healthy_indices) > 0:
            axes[0].plot(valid_healthy_indices, losses_healthy[valid_healthy_indices], label='Error Sano', color='blue',
            ...
            local_logger_plot.warning("No hay errores sanos positivos para graficar en escala log.")
            axes[0].plot([], [], label='Error Sano')  # Para que aparezca la leyenda
        axes[0].set_title('Datos Sanos (Línea Base)')
    ...
    if losses_damage is not None and losses_damage.size > 0:
        ...
        if len(valid_damage_indices) > 0:
            axes[1].plot(valid_damage_indices, losses_damage[valid_damage_indices], label='Error Daño', color='red',
                         alpha=0.7, linewidth=0.5)  # Línea más fina
            axes[1].set_yscale('log')
            ...
            local_logger_plot.warning("No hay errores de daño positivos para graficar en escala log.")
            axes[1].plot([], [], label='Error Daño')  # Para leyenda
        axes[1].set_title('Datos con Daño')
        axes[1].set_xlabel('Índice de Ventana (Tiempo)')
    ...
    try:
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        local_logger_plot.info(f"Gráfico de series de tiempo del error guardado en: {filename}")
    except Exception as e:
        local_logger_plot.error(f"Error al guardar el gráfico de series de tiempo {filename}: {e}")
    plt.close()
...
def plot_error_heatmap(losses_healthy_per_sensor, losses_damage_per_sensor, output_dir):
    ...
    """
    local_logger_plot.info("Generando mapas de calor del error...")

    # Calcular vmin y vmax globales excluyendo posibles ceros si usamos escala log
    vmin = np.min([np.min(losses_healthy_per_sensor[losses_healthy_per_sensor > 0]),
                   np.min(losses_damage_per_sensor[
                              losses_damage_per_sensor > 0])]) if losses_healthy_per_sensor.size > 0 and losses_damage_per_sensor.size > 0 else 1e-9
    vmax = np.max([np.max(losses_healthy_per_sensor),
                   np.max(
                       losses_damage_per_sensor)]) if losses_healthy_per_sensor.size > 0 and losses_damage_per_sensor.size > 0 else 1.0
    ...
    fig.suptitle('Mapa de Calor del Error (Sensor vs. Tiempo)', fontsize=16)

    sensor_labels = [f'Sensor {i + 1}' for i in range(losses_healthy_per_sensor.shape[1])]

    if losses_healthy_per_sensor is not None and losses_healthy_per_sensor.size > 0:
        # Transponer para que los sensores queden en el eje Y
        sns.heatmap(losses_healthy_per_sensor.T, ax=axes[0], cmap='Blues', norm=norm,
                    cbar_kws={'label': 'MSE (Escala Log)'}, xticklabels=10000)  # Mostrar menos etiquetas X
        axes[0].set_title('Datos Sanos (Línea Base)')
    ...
    if losses_damage_per_sensor is not None and losses_damage_per_sensor.size > 0:
        sns.heatmap(losses_damage_per_sensor.T, ax=axes[1], cmap='Reds', norm=norm,
                    cbar_kws={'label': 'MSE (Escala Log)'}, xticklabels=10000)  # Mostrar menos etiquetas X
        axes[1].set_title('Datos con Daño')
        axes[1].set_yticklabels(sensor_labels, rotation=0)
        axes[1].set_ylabel('Sensor')
        axes[1].set_xlabel('Índice de Ventana (Tiempo aprox.)')  # Cambiar label
    else:
        axes[1].set_title('Datos con Daño (No disponibles)')
    ...
    try:
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        local_logger_plot.info(f"Mapa de calor del error guardado en: {filename}")
    except Exception as e:
        local_logger_plot.error(f"Error al guardar el mapa de calor {filename}: {e}")
    plt.close()


# --- NUEVO GRÁFICO ESTADÍSTICO DETALLADO ---
def plot_error_statistics_per_sensor(losses_healthy_per_sensor, losses_damage_per_sensor, output_dir):
    """
    Genera boxplots comparativos de la distribución de errores por sensor
    ...
    if losses_healthy_per_sensor is None or losses_damage_per_sensor is None or \
        ...
        local_logger_plot.warning("Datos de pérdidas por sensor insuficientes para gráfico estadístico.")
        return

    local_logger_plot.info("Generando gráfico estadístico de errores por sensor (Boxplots)...")
    num_sensors = losses_healthy_per_sensor.shape[1]
    sensor_labels = [f'Sensor {i + 1}' for i in range(num_sensors)]

    # Preparar datos para seaborn boxplot
    data_healthy_list = []
    data_damage_list = []
    for i in range(num_sensors):
        ...
        data_healthy_list.append(healthy_positive)
        data_damage_list.append(damage_positive)

    fig, axes = plt.subplots(1, 2, figsize=(18, 7), sharey=True)  # Compartir eje Y
    fig.suptitle('Distribución Estadística del Error de Reconstrucción por Sensor', fontsize=16)

    # Boxplot para Datos Sanos
    bp_healthy = axes[0].boxplot(data_healthy_list, labels=sensor_labels, patch_artist=True, showfliers=False)
    for patch, i in zip(bp_healthy['boxes'], range(num_sensors)):
        patch.set_facecolor(sensor_colors[i % len(sensor_colors)])
        patch.set_alpha(0.6)
    for median in bp_healthy['medians']: median.set(color='black', linewidth=1.5)
    axes[0].set_title('Datos Sanos (Línea Base)')
    axes[0].set_ylabel('Error Cuadrático Medio (MSE) - Escala Log')
    axes[0].set_yscale('log')
    axes[0].grid(True, linestyle=':')

    # Boxplot para Datos con Daño
    bp_damage = axes[1].boxplot(data_damage_list, labels=sensor_labels, patch_artist=True, showfliers=False)
    for patch, i in zip(bp_damage['boxes'], range(num_sensors)):
        patch.set_facecolor(sensor_colors[i % len(sensor_colors)])
        patch.set_alpha(0.8)  # Ligeramente más opaco
    for median in bp_damage['medians']: median.set(color='black', linewidth=1.5)
    axes[1].set_title('Datos con Daño')
    # axes[1].set_ylabel('Error Cuadrático Medio (MSE) - Escala Log') # Eje Y compartido
    axes[1].set_yscale('log')
    axes[1].grid(True, linestyle=':')

    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    filename = os.path.join(output_dir, "error_statistics_per_sensor_boxplot.png")
    try:
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        local_logger_plot.info(f"Gráfico estadístico de errores guardado en: {filename}")
    except Exception as e:
        local_logger_plot.error(f"Error al guardar el gráfico estadístico {filename}: {e}")
    plt.close(fig)
...
def run_inference_and_plot(model_dir, base_healthy_dir, damage_data_dir=None):
    ...
    try:
        ...
        # *** CORRECCIÓN ***
        # Esta es la línea que faltaba y causaba el NameError
        all_healthy_data_raw, max_len_healthy = load_data_from_dir(base_healthy_dir, num_nodes)

        # --- INICIO DE LA CORRECCIÓN DE ERROR ---
        # Manejar el caso en que los datos sanos no se carguen
        if all_healthy_data_raw is None:
            raise ValueError(
                f"Error fatal: No se pudieron cargar los datos sanos (healthy data) desde {base_healthy_dir}. Verifique la ruta y los archivos.")
        ...
        damage_losses = None  # Necesitamos guardar las pérdidas globales también
        ...
        if damage_data_dir:
            ...
            if all_damage_data_raw is not None:
                ...
                if len(damage_dataset) > 0:
                    ...
                    # Recuperar TODAS las salidas de perform_inference
        ...
        # Plotear muestra de reconstrucción sana (si hay datos)
        if healthy_originals is not None and len(healthy_originals) > 0:
            plot_idx_healthy = np.random.randint(max(1, len(healthy_originals) // 4),
                                                 max(2, 3 * len(healthy_originals) // 4))  # Asegurar índice válido
            plot_reconstruction_sample(
                healthy_originals[plot_idx_healthy],
                healthy_reconstructions[plot_idx_healthy],
                scaler,
                plot_idx_healthy,
                results_dir,
                "healthy"
        ...
        # Plotear muestra de reconstrucción con daño (si hay datos)
        if damage_originals is not None and len(damage_originals) > 0:
            plot_idx_damage = np.random.randint(max(1, len(damage_originals) // 4),
                                                max(2, 3 * len(damage_originals) // 4))  # Asegurar índice válido
            plot_reconstruction_sample(
                damage_originals[plot_idx_damage],
                damage_reconstructions[plot_idx_damage],
                scaler,
                plot_idx_damage,
                results_dir,
                "damage"
        ...
        if damage_mse_mean_per_sensor is not None and damage_losses_per_sensor is not None and damage_losses is not None:
            # Gráficos de comparación que ya teníamos
            plot_mse_comparison(
            ...
            # --- LLAMADA A NUEVOS GRÁFICOS ---
            logger.info("Generando gráficos adicionales para análisis de artículo...")

            plot_error_distribution_kde(
                healthy_losses,  # (n_samples,)
                damage_losses,  # (n_samples,)
                results_dir
            )
            ...
            # --- LLAMADA AL NUEVO GRÁFICO ESTADÍSTICO ---
            ...
            logger.info(
                "No hay datos de daño procesados o completos, omitiendo gráficos de comparación, localización y estadísticos.")
...
if __name__ == '__main__':
    ...
    if not os.path.isdir(trained_model_directory):
        ...
        logger.warning(
            f"Directorio de datos de daño proporcionado pero no encontrado: {damage_data_directory}. Ejecutando solo con datos sanos.")
