#!/usr/bin/env python3
"""
train_potato_blight.py  (versión GPU‑ready)
...
Ejemplo de uso desde terminal:
    ...
    python Potato.py --data_dir "D:/Python_proyectos_2025/Agricultura/archive/DATA"
    # Añade --skip_tflite si no deseas exportar el modelo TFLite al final
"""
...
if physical_gpus:
    try:
        # Evita que TensorFlow reserve toda la VRAM al iniciar
        for gpu in physical_gpus:
            tf.config.experimental.set_memory_growth(gpu, True)
        # Usa solo la primera GPU disponible
        tf.config.set_visible_devices(physical_gpus[0], "GPU")
        DEVICE = "/GPU:0"
        print("✔ Entrenamiento forzado en", DEVICE)
    except RuntimeError as e:
        print("⚠️  No se pudo fijar GPU:", e)
        DEVICE = "/CPU:0"
else:
...
def prepare_datasets(
    ...
    # Primer pase sin shuffle para computar pesos de clase
    ...
    class_weight = {
        i: (1.0 / counts[i]) * (counts.sum() / len(counts)) for i in range(len(counts))
    }

    # Recarga con shuffle para el entrenamiento real
...
def evaluate_and_save(best_model_path, val_ds, final_model_path="final_model.keras"):
...
def main():
    parser = argparse.ArgumentParser(description="Train Potato Blight Classifier")
    parser.add_argument("--data_dir", required=True, help="Path to dataset directory")
    parser.add_argument("--skip_tflite", action="store_true", help="Do not export TFLite model")
    args = parser.parse_args()

    set_seeds()

    train_ds, val_ds, class_names, class_weight = prepare_datasets(args.data_dir)
    ...
    # Fase 1: Entrenamiento inicial (feature extraction)
    ...
    best_model_path = best_ft_ckpt if os.path.exists(best_ft_ckpt) else best_feature_ckpt
